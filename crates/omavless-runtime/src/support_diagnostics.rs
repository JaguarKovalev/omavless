...
mod tests {
    ...
    fn host_policy_counts_match_actual_python_reference_for_bundles_and_shapes() {
        ...
        // Developer-only oracle; no private store, lifecycle helper or network.
        // The Python reference receives only checked-in credential-free text.
        let script = "import json,sys,backend\nprint(json.dumps([[backend.yaml_sequence_count(backend.yaml_top_level_block(t,'rules')),backend.yaml_mapping_count(backend.yaml_top_level_block(t,'rule-providers'))] for t in json.load(sys.stdin)]))";
        ...
        assert!(output.status.success(), "support policy oracle failed");
        let expected: Vec<[u32; 2]> = serde_json::from_slice(&output.stdout).unwrap();
        assert_eq!(expected.len(), cases.len());
        for (case, expected) in cases.iter().zip(expected) {
            let actual = policy_counts(case, false).unwrap();
            assert_eq!([actual.rules, actual.providers], expected);
        }
    }
