...
receipt. Its [read-side startup barrier](docs/testing/R5_LOGIN_STARTUP_BARRIER.md)
...
The [fresh native local observation](docs/testing/R5_NATIVE_RUNTIME_OBSERVATION.md)
adds strict visible inventories and authenticated owned-controller facts through
...
controller, while observation remains read-only. Production preflight now checks
the same configured predicate before adoption, with exact peer and stale-state
