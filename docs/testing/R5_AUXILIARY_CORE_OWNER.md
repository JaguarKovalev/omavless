...
The local host follow-up now binds a private auxiliary slot to the native host,
drains before ordinary/remote-completion mutations outside the dispatcher mutex,
and revokes it during scheduler shutdown. Host start/stop also asserts an empty
slot, so bypassing outer admission cannot silently start a competing core.
`visibleMihomoCount` remains the raw named-process count. The additive bounded
`ownedAuxiliaryMihomoCount` is its explicitly proven subset (zero or one);
frontend parsing accepts older responses without it as zero. Unknown extra
cores still invalidate tunnel health. Neither count proves TUN ownership.

Host follow-up gate: 554 runtime library tests passed, 5 pre-existing opt-ins
ignored; 16 observation/action and 7 presentation JavaScript tests passed;
strict all-target runtime clippy passed. A TERM-refusing disposable test child
proves status can respond while cancellation drains, before a mode mutation.
This still does not register the actual subscription-probe job or enable UI.
